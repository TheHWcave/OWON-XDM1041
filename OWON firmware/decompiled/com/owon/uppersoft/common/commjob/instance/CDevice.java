...
import com.owon.uppersoft.common.commjob.instance.ComparableDevice;
...
public class CDevice {
    ...
    public static List<IDevice> scanMatchedDevices(short idVendor, short idProduct) throws USBException {
        LibusbJava.usb_init();
        LibusbJava.usb_find_busses();
        LibusbJava.usb_find_devices();
        Usb_Bus bus = LibusbJava.usb_get_busses();
        ...
        while (bus != null) {
            ...
            while (device != null) {
                ...
                if (devDesc.getIdVendor() == idVendor && devDesc.getIdProduct() == idProduct) {
                    ComparableDevice cd = ComparableDevice.getInstance(device, "");
                    long handle = LibusbJava.usb_open((Usb_Device)device);
    ...
    public static void simulateOpen(CDevice device, IDevice id) throws USBException {
        ...
        CDevice.simulateOpen(device, dev, id.getBConfigurationValue(), id.getBInterfaceNumber(), id.getBAlternateSetting());
    }
    ...
    public static void simulateOpen(CDevice device, Usb_Device dev, int configuration, int interface_, int altinterface) throws USBException {
        ...
        if (device.resetOnFirstOpen & !device.resetDone) {
            ...
            CDevice.simulateOpen(device, dev, configuration, interface_, altinterface);
        }
    }

    public static long direct_open(Usb_Device device, int configuration, int interface_, int altinterface) throws USBException {
        long usb_dev_handle = LibusbJava.usb_open((Usb_Device)device);
        ...
        if (altinterface >= 0 && LibusbJava.usb_set_altinterface((long)usb_dev_handle, (int)altinterface) < 0) {
            try {
                if (LibusbJava.usb_release_interface((long)usb_dev_handle, (int)interface_) < 0) {
                    throw new USBException("LibusbJava.usb_release_interface: " + LibusbJava.usb_strerror());
                }
            }
            catch (USBException uSBException) {}
    ...
    public static void direct_contact_test(long usb_dev_handle, int WriteEndpoint, int ReadEndpoint) {
        int len = LibusbJava.usb_bulk_write((long)usb_dev_handle, (int)WriteEndpoint, (byte[])"START".getBytes(), (int)5, (int)2000);
        if (len < 0) {
        }
        byte[] twv = new byte[12];
        len = LibusbJava.usb_bulk_read((long)usb_dev_handle, (int)ReadEndpoint, (byte[])twv, (int)twv.length, (int)2000);
        System.out.println(len);
        int i = 0;
        while (i < len) {
            System.out.print(String.valueOf(Integer.toHexString(0xFF & twv[i])) + " ");
            ++i;
        }
        twv = new byte[1024];
        i = 0;
    ...
    public static void direct_reset(long usb_dev_handle, int interface_) throws USBException {
        LibusbJava.usb_release_interface((long)usb_dev_handle, (int)interface_);
        if (LibusbJava.usb_reset((long)usb_dev_handle) < 0) {
            throw new USBException("LibusbJava.usb_reset: " + LibusbJava.usb_strerror());
        }
    }
    ...
    public static void direct_close(long usb_dev_handle, int interface_) throws USBException {
        LibusbJava.usb_release_interface((long)usb_dev_handle, (int)interface_);
        if (LibusbJava.usb_close((long)usb_dev_handle) < 0) {
            throw new USBException("LibusbJava.usb_close: " + LibusbJava.usb_strerror());
        }
    }
}
