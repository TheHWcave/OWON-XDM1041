...
import com.owon.uppersoft.common.comm.ComparableDevice;
    ...
    public static List<IDevice> scanMatchedDevices(short idVendor, short idProduct) throws USBException {
        ...
        while (bus != null) {
            ...
            while (device != null) {
                ComparableDevice cd;
                Usb_Device_Descriptor devDesc = device.getDescriptor();
                if (devDesc.getIdVendor() == idVendor && devDesc.getIdProduct() == idProduct && (cd = ComparableDevice.getInstance(device, "")) != null) {
                    devices.add(cd);
                }
                device = device.getNext();
            }
